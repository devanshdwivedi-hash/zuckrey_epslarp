 ...
 * Dual-State Game Loop: Idle Screen | Active Playing | Game Over
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Player Character (Low-poly runner)
  const runner = {
    x: 25,
    y: 85,
    w: 14,
  ...
  // Obstacles (AI Server Racks & Cacti)
  let obstacles = [];
  let obstacleTimer = 0;
  let gameSpeed = 3.5;

  // Input Handling: Spacebar & Click
  ...
  function update() {
    if (state === 'PLAYING') {
      ...
      gameOverTimer++;
      if (gameOverTimer > 120) { // 2 seconds at ~60fps
        state = 'IDLE';
      }
    }
  }
  ...
  function render() {
    // 1. Dark Screen Background & Retro Grid Line
    ...
    if (state === 'IDLE') {
      ...
      // Draw Low-Poly Runner (Toxic Green Polygon Head & Body)
      ctx.fillStyle = '#33ff00';
      ...
      // Draw Obstacles (Cacti / AI Server Racks)
      obstacles.forEach(obs => {
        if (obs.type === 'SERVER') {
          // AI Server Rack (Dark Blue-Green with LED lights)
