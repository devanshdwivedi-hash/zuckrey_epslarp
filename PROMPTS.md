...
# Conversation Ids
2668f4c8-2446-4fc1-84dc-b782cd984cdf
c116d16d-1ff4-4538-843f-4e8e2f041779

# 🤖 Project Directives & AI Agent Prompts

This document serves as the master prompt blueprint for building the **Autonomous AI Content Agent**. It contains structured developer directives for both **Dev A (Intelligence Pipeline)** and **Dev B (Infrastructure, Database, API & Scheduler)**.

---

## 📋 Table of Contents
1. [Dev A: Content & LLM Intelligence Pipeline](#-dev-a-content--llm-intelligence-pipeline)
2. [Dev B: Storage, Memory & Semantic Deduplication](#-dev-b-storage-memory--semantic-deduplication)
3. [Dev B: FastAPI Web Server & Public Feed](#-dev-b-fastapi-web-server--public-feed)
4. [Dev B: Database Layer & Persistence](#-dev-b-database-layer--persistence)
5. [Dev B: Scheduler & Autonomous Execution Loop](#-dev-b-scheduler--autonomous-execution-loop)
6. [Dev B: External Dispatcher Module (Webhooks)](#-dev-b-external-dispatcher-module-webhooks)
7. [Master Integration & Handshake Pipeline](#-master-integration--handshake-pipeline)

---

## 🧠 Dev A: Content & LLM Intelligence Pipeline

### Phase 1: Setup & Contracts (`config/`, `src/intelligence/schemas.py`)
- Define persona in `config/persona_config.py` ("AI Security Researcher"). Set sharp voice, strict technical focus, and hard rejection rules for fluff/hype.
- Ensure Pydantic schemas in `schemas.py` match:
  - **`RawTopic`**: `title`, `summary`, `url`, `source_name`, `article_published_at`
  - **`EditorialDecision`**: `decision` ("PUBLISH" | "REJECT"), `score` (1-10), `reason`
  - **`GeneratedPost`**: `title`, `content`, `selection_reason`, `why_relevant_now`, `sources`

### Phase 2: Data Ingestion (`src/scrapers/`)
- Implement scrapers inheriting from `BaseScraper` (`base.py`):S
  - `hn_scraper.py`: HackerNews API + AI/Security keyword filter.
  - `arxiv_scraper.py`: arXiv RSS (`cs.AI`, `cs.CR`).
  - `rss_scraper.py`: Tech blog feeds.
- Output clean lists of `RawTopic`.

### Phase 3: Editorial Evaluator (`src/intelligence/evaluator.py`)
- Implement `evaluate_topic(topic: RawTopic) -> EditorialDecision` using OpenAI Structured Outputs (`pydantic`).
- Enforce strict persona prompt: approve technical safety/vulnerabilities, reject PR/marketing hype.

### Phase 4: Post Generation (`src/intelligence/generator.py`)
- Implement `generate_post(topic: RawTopic) -> GeneratedPost` using structured outputs.
- Enforce output fields matching `GeneratedPost`:
  - `title`: Clean headline for the post.
  - `content`: Written analysis/post in persona voice.
  - `selection_reason`: Clear explanation of why this topic met editorial criteria over others.
  - `why_relevant_now`: Explanation of timeliness and immediate technical relevance.
  - `sources`: List containing the original URL (`[topic.url]`).

---

## 💾 Dev B: Storage, Memory & Semantic Deduplication

### Module: `src/memory/`
1. **`src/memory/embeddings.py`**:
   - Implement `get_embedding(text: str) -> list[float]` using OpenAI (`text-embedding-3-small`) or `sentence-transformers`.
2. **`src/memory/deduplicator.py`**:
   - Implement `is_duplicate(candidate_vector: list[float], published_vectors: list[list[float]], threshold: float = 0.85) -> bool`.
   - Use cosine similarity (`numpy` or `scipy`). Return `True` if any similarity score $\ge$ `threshold`.

---

## 🌐 Dev B: FastAPI Web Server & Public Feed

### Module: `src/api/`
1. **`src/api/routes.py`**:
   - Create `router = APIRouter()`.
   - Implement `GET /feed`: Fetch published posts from DB (`published_posts` table), sort by `created_at` descending, limit to top 50, and return a list of JSON objects matching:
     ```json
     {
       "title": "string",
       "content": "string",
       ...
       "sources": ["string"],
       "created_at": "2026-08-09T12:40:00Z"
     }
     ```
2. **`src/api/main.py`**:
...
## 🗄️ Dev B: Database Layer & Persistence

### 1. Database Connection (`src/db/database.py`)
- Read `DATABASE_URL` from `.env` (default to `sqlite:///./agent.db`).
- If PostgreSQL (`postgres://` or `postgresql://`), convert connection string for SQLAlchemy compatibility if necessary.
- Add `connect_args={"check_same_thread": False}` **only** when running SQLite engine.
- On server startup (`src/api/main.py` lifespan/startup), run `Base.metadata.create_all(bind=engine)`.

### 2. Database Models & CRUD (`src/db/models.py`)
- **`PublishedPost`**: `id`, `title`, `content`, `selection_reason`, `why_relevant_now`, `sources` (JSON), `vector_embedding` (JSON/Pickle), `created_at` (DateTime with timezone).
- **`RejectedPost`**: `id`, `title`, `rejection_reason`, `score`, `created_at` (DateTime with timezone).
- CRUD Functions:
  - `save_published_post(post: GeneratedPost, vector: list[float]) -> PublishedPost`
  - `save_rejected_post(topic_title: str, reason: str, score: int)`
  - `get_all_embeddings() -> list[list[float]]`
  - `get_published_feed() -> list[PublishedPost]`

---

## ⏰ Dev B: Scheduler & Autonomous Execution Loop

### Module: `src/scheduler/cron.py`
1. Initialize `BackgroundScheduler` (or serverless `/api/cron` route handler for Vercel).
2. Schedule `run_autonomous_loop()` to run every 30–60 minutes.
3. Execution Lifecycle:
   - Ingest raw topics via Dev A's scrapers.
   - Convert topic text to vector embeddings.
   - Filter out duplicate topics using `is_duplicate()` memory check.
   - Send remaining topics to LLM evaluator & post generator.
   - Save generated posts to `published_posts` and rejected topics to `rejected_posts`.
4. Wrap loop in `try/except` blocks to handle network timeouts or rate limits resiliently.

---

## 📢 Dev B: External Dispatcher Module (Webhooks)

### Module: `src/api/dispatcher.py`
1. Implement `dispatch_post(post: GeneratedPost)` function using `httpx`.
2. Post payload to `DISCORD_WEBHOOK_URL` defined in `.env`.
3. Format payload: `title`, `content` (truncated to 2000 chars), `selection_reason`, and `sources`.
4. Trigger `dispatch_post(post)` after database commit in `cron.py` inside a non-blocking `try/except` wrapper.

---

## 🤝 Master Integration & Handshake Pipeline

### Module: `src/intelligence/pipeline.py`
Unified entry point bundling Dev A's pipeline for Dev B's scheduler invocation:
...
async def run_discovery_and_evaluation(
    ...
    """
    1. Aggregation: Run scrapers (HN, arXiv, RSS) to gather candidate RawTopics.
    2. Deduplication: Generate embeddings and run is_duplicate_fn check.
    3. Evaluation: Pass candidates to evaluate_topic() for APPROVED/REJECTED split.
    4. Generation: Call generate_post() for all approved topics.
    5. Return tuple of (accepted_posts, rejected_items).
    """
    # 🤖 Project Specification & AI Master Directives: Autonomous AI Content Agent
...
### The Situation
[cite_start]Every day, thousands of AI-generated posts appear on social platforms[cite: 1]. [cite_start]Almost all of them exist because a human wrote the initial prompt[cite: 1]. [cite_start]While today's models are excellent writers, they are rarely autonomous creators[cite: 2].

### The Challenge
[cite_start]Build an autonomous AI and technology persona that operates without human instructions[cite: 3]. Once initialized, the agent independently:
1. [cite_start]**Discovers Topics:** Scans live information sources (web, RSS, arXiv, APIs)[cite: 3].
2. [cite_start]**Editorial Judgment:** Evaluates content quality and intentionally rejects topics that do not meet strict publishing standards[cite: 3].
3. [cite_start]**Consistent Persona:** Maintains a recognizable identity and writing voice (e.g., *AI Security Researcher*)[cite: 3].
4. [cite_start]**Vector Memory:** Remembers previously published content to eliminate duplicates[cite: 3].
5. [cite_start]**Autonomous Execution:** Continuously loops and publishes over time via automated cron execution[cite: 3].
...
```text
               ┌─────────────────────────────────────────┐
               │    render Cron / Background Scheduler    │
               └────────────────────┬────────────────────┘
                                    │ (Triggers every 30 mins)
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ DEV A: Content & Intelligence Pipeline                                 │
│                                                                        │
│  [ Scrapers (HN / arXiv / RSS) ] ──► [ Raw Topic Discovery ]           │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ DEV B: Memory & Deduplication                                          │
│                                                                        │
│  [ Vector Embedding Generator ] ──► [ Cosine Similarity Check ]        │
│                                           │                            │
│                  ┌────────────────────────┴────────────────────────┐   │
│                  ▼                                                 ▼   │
│            [ DUPLICATE ]                                     [ NEW TOPIC ] │
...
└────────────────────────────────────────────────────────────────────┼───┘
                                                                     ...
                                                                     ▼
┌────────────────────────────────────────────────────────────────────────┐
│ DEV A: Editorial & Post Generation                                     │
│                                                                        │
│  [ LLM Editorial Evaluator ] ──► Reject? ──► [ Save to rejected_posts] │
│                  │                                                     │
│            (Approved)                                                  │
│                  ▼                                                     │
│  [ Persona Post Generator ]                                            │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│ DEV B: Persistence, API & Dispatcher                                   │
│                                                                        │
│  1. [ Save Post to Supabase DB ] ──► Stamps `created_at` (Agent Sync)  │
│  2. [ Webhook Dispatcher ]       ──► Pushes post to Discord / Slack    │
│  3. [ FastAPI GET /feed ]        ──► Serves UI sorted by `created_at`  │
└────────────────────────────────────────────────────────────────────────┘


#THES ARE PROMPTS GENERATED BY GEMINI AND GIVEN TO THE ANTIGRAVITY# 📜 User Prompts & Directive Execution Log
